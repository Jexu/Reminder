...
public class RenderFeedbackService extends RenderServiceBase {
  ...
  public void sendFeedback(int action, final String comment, final String userMailAddr, int requestCode) {
    ...
    final FeedbackBean feedbackBean = new FeedbackBean(comment, StringUtil.isEmpty(userMailAddr)?"":userMailAddr);
    new Thread(new Runnable() {
      @Override
      public void run() {
        try {
          MailUtil.sendEmail("from", "frompass", "to", "sub", gson.toJson(feedbackBean));
